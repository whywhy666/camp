...
type CreateMemberRequest struct {
	Nickname string   `json:"nickname" form:"nickname" binding:"required,min=4,max=20"` // required，不小于 4 位 不超过 20 位
	Username string   `json:"username" form:"username" binding:"required"`              // required，只支持大小写，长度不小于 8 位 不超过 20 位
	Password string   `json:"password" form:"password" binding:"required,min=8,max=20"` // required，同时包括大小写、数字，长度不少于 8 位 不超过 20 位
	UserType UserType `json:"usertype" form:"usertype" binding:"required"`              // required, 枚举值
}
